...
- [Launcher 內部機制](#launcher-內部機制)
  ...
  - [離線支援：已安裝、沒異動的工具離線也能開](#離線支援已安裝沒異動的工具離線也能開)
  - [Launcher 自我更新](#launcher-自我更新)
- [網頁端機制](#網頁端機制)
...
1. 下載 Launcher：[real-toolbox-launcher.exe](https://github.com/real1027/real-toolbox/releases/latest/download/real-toolbox-launcher.exe)（這個網址永遠指向最新版，見下方「Launcher 自我更新」）
...
### 離線支援：已安裝、沒異動的工具離線也能開

實際使用情境：產線機台有時候真的完全沒有網路，這時候還是得能開已經裝好的工具。

第一步「讀 manifest.json」原本完全沒有離線 fallback——連不上網就整個失敗。現在改成：每次成功連線抓到 manifest.json，都會順手存一份到本機（`APP_DIR/manifest_cache.json`）；下次連線失敗時，改用這份快取，而不是直接報錯。

搭配版本比對／內容指紋比對本來就是「連不到就安全地跳過，退回用本機記錄判斷」的設計（見上面兩節），整體效果是：

- **已經裝好、版本沒變的工具**：完全不需要網路就能啟動，包含連不到 GitHub Pages（用快取的 manifest）、連不到內網 GitLab（版本/指紋比對安全跳過）。
- **需要重新下載或更新的工具**：離線環境沒辦法完成，會跳出清楚的錯誤訊息說明是連線問題，而不是一段看不懂的技術錯誤。
- **這台機器從來沒有成功連線過一次**：沒辦法離線完成第一次使用，因為連「有哪些工具」都無從得知——這是唯一无法繞過的限制。

### Launcher 自我更新

Launcher 本身也會檢查自己是不是最新版，做法跟工具版本比對的邏輯一樣，只是問的對象換成 Launcher 自己在 GitHub 上的 Release：

- `launcher.py` 裡有一個 `LAUNCHER_VERSION` 常數，每次真的有改動就手動 bump，並且**真的去 GitHub 建一個新的 Release tag**（例如 `launcher-v1.1.0`），把新 build 的 exe 附上去（不像早期的 `launcher-v1.0.0` 只是重複覆蓋同一個 tag 的附件）。
- Launcher 每次啟動工具時（有節流，一天最多檢查一次，記錄在 `%LOCALAPPDATA%\real-toolbox\config.json`），會用跟 GitLab permalink 一樣的技巧問 GitHub：對 `https://github.com/real1027/real-toolbox/releases/latest` 發 HEAD request，從 redirect 的 `Location` header 讀出目前真正的最新 tag。
- 發現版號不一樣時，會從 `https://github.com/real1027/real-toolbox/releases/latest/download/real-toolbox-launcher.exe`（GitHub 也有這種「永遠指向最新版附件」的固定網址，跟 GitLab permalink 是同樣的概念）下載新版，然後**自動在背景把自己換掉**，全程不用使用者做任何事。

**這次要求做到「完全自動靜默更新」，不是只跳提醒**——這牽涉到一個 Windows 特性：一個正在執行的程式沒辦法直接覆蓋自己的 exe 檔案，所以做法是：下載新版之後，另外開一個獨立的背景小程序（用同一支 exe，帶一個內部隱藏參數 `--apply-update`），等目前這個程序真正結束、釋放檔案鎖定之後，才把新版蓋上去。這個背景小程序會重試到 10 分鐘（每 2 秒試一次），不是重試個幾秒就放棄——實測過程中發現：如果這次啟動剛好跳出了錯誤訊息框（例如工具還沒上架），程式要等使用者按掉那個訊息框才會真的結束，這個等待時間使用者可能拖很久，重試視窗抓太短會幾乎每次都失敗。全程對使用者完全靜默（不會跳出「更新中」或「更新完成」之類的視窗），更新從下一次點擊才會生效，這次點擊一律用目前這個版本正常跑完，不會被自我更新卡住或打斷。

如果自我更新這次失敗了（例如檔案一直被占用超過 10 分鐘），也不會有任何提示，下次啟動再檢查到版號不一樣，就會再試一次——這個機制設計成可以放著不管、自己收斂。

**怎麼知道自己裝的是哪一版？** 一開始加自我更新時完全沒想到這件事——結果發現使用者連「目前是哪一版」都沒地方看。後來補上兩個管道：Launcher 跳出的**每一個**訊息框（設定完成、錯誤訊息等等）標題都會固定顯示版本號，不用特別做什麼；也可以直接執行 `real-toolbox-launcher.exe --version` 主動查詢。

**更新成功後會不會有任何提示？** 會。原本設計成完全靜默（連成功了都不講），後來使用者實際用過反應「點了啟動、工具開起來了，但完全不知道 Launcher 到底有沒有更新」，所以改成：**更新成功後**會跳出一個訊息框，寫「已自動更新到 vX.Y.Z」，並附上這次更新的說明文字（直接讀取那個 GitHub Release 自己的說明欄位，不用另外維護一份）。更新失敗的話還是完全不提示（沒什麼好講的，下次啟動會再試一次）。

有個先天的銜接限制：因為執行「換檔」這個動作的，是換之前那個**舊版**的程式碼，所以「這次更新完會不會顯示提示」取決於换你之前那個版本有沒有這個功能——換句話說，**加上這個功能的這一版本身在被舊版 Launcher 更新過來的那一次，不會顯示提示**（實測驗證過：真的用已上線的 v1.2.0 去更新到 v1.3.0，成功換檔但沒跳提示，因為執行換檔的是 v1.2.0 的舊程式碼）。從這一版開始，之後每一次更新才會正常顯示提示。
